...
trait Perfex_saas_subscription_trait
{
    ...
    public function generate_company_invoice($clientid, $packageid, $extra_custom_data = [])
    {
        ...
        $future_custom_limits  = (object)($old_customization->validatable_limits ?? new stdClass());
        ...
        if ($old_invoice) {
            ...
            $_package->custom_limits = $future_custom_limits;
            ...
            if ($max_instance_limit > 0 && $total_instances > 0) {
                ...
                foreach ($companies as $company) {
                    ...
                    // Add custom limits purchase if any to have the real new quota
                    $company->package_invoice->custom_limits = $future_custom_limits;
        ...
        // Update the customization to the new validated one
        perfex_saas_get_or_save_client_metadata($clientid, ['custom_limits' => $future_custom_limits]);
        ...
        if (!empty($custom_limits)) {
            foreach ($custom_limits as $resources => $quantity) {
                ...
                if ($old_package) {
                    ...
                    if ($resources === 'tenant_instance') {
                        $old_quota = (int) $old_package->metadata->max_instance_limit ?? -1;
                        $new_limit = (int)($package->metadata->max_instance_limit ?? -1);
                    }

                    if ($is_storage) {
                        $old_quota = (int) $old_package->metadata->storage_limit->size ?? -1;
                        $new_limit = (int)($package->metadata->storage_limit->size ?? -1);
                    }
