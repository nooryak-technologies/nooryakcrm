...
if (!function_exists('perfex_saas_get_cpanel')) {
    ...
    {
        $CI = &get_instance();
        $user = trim((string)get_option('perfex_saas_cpanel_username'));
        $login_domain = trim((string)get_option('perfex_saas_cpanel_login_domain'));
        $port = trim((string)get_option('perfex_saas_cpanel_port'));

        $raw_password = (string)get_option('perfex_saas_cpanel_password');
        $decrypted_password = $CI->encryption->decrypt($raw_password);
        // Some installs may store this value in plain text (legacy/manual updates),
        // so gracefully fallback instead of passing false/empty and causing HTTP 401.
        $password = is_string($decrypted_password) && $decrypted_password !== ''
            ? $decrypted_password
            : trim($raw_password);

        // Set appropriately if user have added custom cpanel db_prefix
        $db_prefix = get_option('perfex_saas_cpanel_db_prefix') ?? '';
        $db_prefix = trim((string)$db_prefix);
