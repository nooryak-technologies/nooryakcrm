...
if ($client_bridge) {
    ...
    if ($is_tenant) {
        ...
        // If a tenant owner logs in from the customer form with the same email as a tenant admin,
        // switch to the admin dashboard automatically for a smoother first-login experience.
        hooks()->add_action('after_contact_login', function () use ($CI) {
            if (!is_client_logged_in() || is_admin()) {
                return;
            }

            $contact_id = (int) get_contact_user_id();
            if ($contact_id < 1) {
                return;
            }
            ...
            if (perfex_saas_tenant_admin_autologin((int)$staff->staffid)) {
                redirect(admin_url());
            }
        });
