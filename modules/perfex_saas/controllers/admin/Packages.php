...
class Packages extends AdminController
{
    ...
    private function create_or_edit_package($id = '')
    {
        ...
        // Read pricing_feature_lines WITHOUT XSS cleaning to preserve newlines,
        // then strip any HTML tags manually for safety.
        $raw_feature_lines = $this->input->post('metadata', false);
        if (isset($raw_feature_lines['pricing_feature_lines'])) {
            $safe_lines = strip_tags(trim($raw_feature_lines['pricing_feature_lines']));
            $form_data['metadata']['pricing_feature_lines'] = $safe_lines;
        }
