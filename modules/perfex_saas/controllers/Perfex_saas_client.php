...
class Perfex_saas_client extends ClientsController
{
    ...
    public function delete($slug)
    {
        if ($this->input->post() && perfex_saas_contact_can_manage_instances()) {
            ...
            // We dont want the use to remove demo instances.
            $can_delete = $id && !in_array($slug, perfex_saas_demo_instances());
    ...
    public function my_account()
    {
        ...
        if (!$invoice) {
            set_alert('warning', _l('perfex_saas_no_invoice_client_for_client'));
            return $this->client_redirect(site_url());
        }
        ...
        if (!empty($this->input->post())) {
            ...
            $service = $this->updateSubscription($clientid, $invoice, $package);
    ...
    /**
     * Perform common permission check for subscription related endpoints.
     * Will exit the process if permission not met.
     *
     * @return void
     */
    public function ensure_subscription_access()
    {
        ...
        if (perfex_saas_client_is_demo_account()) {
            set_alert('warning', _l('perfex_saas_access_denied_in_demo'));
            redirect(site_url());
            exit;
        }
    }
