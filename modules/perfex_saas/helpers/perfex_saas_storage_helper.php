...
/**
 * Recursively sum all numeric file sizes in a nested array.
 *
 * @param mixed $sizes
 * @return int
 */
function perfex_saas_storage_sum_uploaded_files_sizes($sizes)
{
    if (is_array($sizes)) {
        $total = 0;
        foreach ($sizes as $value) {
            $total += perfex_saas_storage_sum_uploaded_files_sizes($value);
        }
        return $total;
    }
    return (int) $sizes;
...
function perfex_saas_tenant_has_enough_storage($tenant, $uploaded_files = [])
{
    ...
    if (!empty($uploaded_files)) {
        ...
        foreach ($uploaded_files as $upload) {
            $sizes = $upload['size'];
            $total_file_size += perfex_saas_storage_sum_uploaded_files_sizes($sizes);
        }
    }
