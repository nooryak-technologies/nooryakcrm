...
function perfex_saas_deploy_company($company, $silent_mode = false)
{
    try {
        ...
        perfex_saas_setup_tenant_client_contact($company, $dsn);
...
function perfex_saas_remove_company($company, $silent_mode = false)
{
    ...
    try {
        ...
        // Get all table list
        $tables = perfex_saas_get_db_tables_with_prefix($tenant_dbprefix, $db);
        $deleted_tables = 0;
...
function perfex_saas_setup_dsn($dsn, $slug, $source_dsn = [], $return_queries_only = false)
{
    ...
    $hooks_payload = ['dsn' => $dsn, 'dbprefix' => $dbprefix, 'source_dsn' => $source_dsn, 'slug' => $slug, 'tables' => $master_tables, 'queries' => $show_create_queries];
...
function perfex_saas_seed_tenant_from_dsn($company, $dsn, $source_dsn, $dbprefix, $return_queries_only = false)
{
    ...
    $seed_tables = hooks()->apply_filters(
        'perfex_saas_tenant_seeding_tables',
        ['seed_tables' => $seed_tables, 'company' => $company, 'dbprefix' => $dbprefix, 'source_dsn' => $source_dsn, 'source_is_super_admin' => $source_is_super_admin]
    ...
    foreach ($seed_tables as $table) {
        ...
        try {
            // Try catch here incase the seed have tables not in source dsn ; 
            // this happen when some master tables are checked on Tenant Seed settings but not on the seeding tenant source
            $q = "SELECT * FROM $table";

            // Retrieve rows from the master database
            $rows = perfex_saas_raw_query($q, $source_dsn, true, false);
        } catch (\Throwable $th) {
            log_message('error', 'Tenant seed table Error:' . $th->getMessage());
            continue;
        }
        ...
        foreach ($rows as $row) {
            ...
            if (empty($selectors)) {
                $row_cols  = array_keys($row);

                // @since v0.3.6 Use the first 3 cols for idenitification
                $selectors = array_slice($row_cols, 0, min(3, count($row_cols)));
            }
 ...
 * 
 * Will update the existing tenant admin or insert it if missing.
 *
...
function perfex_saas_setup_tenant_admin($tenant, $dsn)
{
    ...
    // Seed the tenant admin login using the registration contact credentials
    $data = [];
    ...
    if (!empty($result->staffid)) {
        $admin_update_data = $data;
        unset($admin_update_data['datecreated']);

        $admin_update_query = $db->update_string($table, $admin_update_data, ['staffid' => $result->staffid]);
        return perfex_saas_raw_query($admin_update_query, $dsn);
    }
...
/**
 * Ensure tenant customer portal has the same login credential as registration.
 *
 * @param object $tenant
 * @param array|string $dsn
 * @return bool
 */
function perfex_saas_setup_tenant_client_contact($tenant, $dsn)
{
    $CI = &get_instance();
    ...
    // Prefer primary contact from registration.
    $contact = $CI->clients_model->get_contact(get_primary_contact_user_id($tenant->clientid));

    // Fallback to staff profile when contact is not available.
    if (!$contact && is_staff_logged_in()) {
        $contact = get_staff(get_staff_user_id());
    }

    if (!$contact || empty($contact->email) || empty($contact->password)) {
        return false;
    }
    ...
    $email = $db->escape($contact->email);
    $existing_contact = perfex_saas_raw_query_row("SELECT `id`,`userid` FROM `$contacts_table` WHERE `email` = $email LIMIT 1", $dsn, true);

    $clientid = (int)($existing_contact->userid ?? 0);
    if ($clientid < 1) {
        $company_name = $tenant->name ?? trim(($contact->firstname ?? '') . ' ' . ($contact->lastname ?? ''));
        ...
        $insert_client_query = $db->insert_string($clients_table, $client_data);
        perfex_saas_raw_query($insert_client_query, $dsn);

        $client_row = perfex_saas_raw_query_row(
            "SELECT `userid` FROM `$clients_table` WHERE `company` = " . $db->escape($company_name) . " AND `datecreated` = " . $db->escape($now) . " ORDER BY `userid` DESC LIMIT 1",
            $dsn,
            true
        );

        $clientid = (int)($client_row->userid ?? 0);
    }
    ...
    if (!empty($existing_contact->id)) {
        $update_contact_data = $contact_data;
        unset($update_contact_data['datecreated']);

        $update_contact_query = $db->update_string($contacts_table, $update_contact_data, ['id' => $existing_contact->id]);
        return (bool) perfex_saas_raw_query($update_contact_query, $dsn);
    }

    $insert_contact_query = $db->insert_string($contacts_table, $contact_data);
    return (bool) perfex_saas_raw_query($insert_contact_query, $dsn);
}
...
function perfex_saas_is_tenant_installation_secured($slug, $dsn_array, $drop_option_table_if_not_fit = false)
{
    ...
    try {
        ...
        $version = perfex_saas_raw_query_row("SELECT `version` FROM $migration_table LIMIT 1;", $dsn_array, true);
        if (!empty($version))
