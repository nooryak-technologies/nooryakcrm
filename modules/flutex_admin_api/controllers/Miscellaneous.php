...
class Miscellaneous extends RestController
{
    ...
    public function privacy_get()
    {
        ...
        $privacyData = $this->knowledge_base_model->get(false, 'privacy-policy');

        if (!empty($privacyData)) {
            $this->response(['message' => _l('data_retrieved_successfully'), 'data' => $privacyData], RestController::HTTP_OK);
        } else {
    ...
    public function general_get()
    {
		...
		$invoice_data = [
		    ...
		    'estimate_due_after' => get_option('estimate_due_after'),
		    ...
		    'proposal_number_prefix' => get_option('proposal_number_prefix'),
		    'proposal_due_after' => get_option('proposal_due_after'),
		    'base_currency' => $this->currencies_model->get_base_currency(),
	    ];
        
        if (!empty($invoice_data)) {
            $this->response(['message' => _l('data_retrieved_successfully'), 'data' => $invoice_data], RestController::HTTP_OK);
        } else {
            $this->response(['message' => _l('data_not_found')], RestController::HTTP_NOT_FOUND);
        }
    }
