...
class Invoices extends RestController
{
    ...
    public function payment_post()
    {
        ...
        try {
            $this->form_validation->set_rules('invoiceid', 'Invoice Id', 'required|max_length[255]');
            ...
            $this->form_validation->set_rules('paymentmode', 'Payment Mode', 'required|max_length[255]');
            ...
            if (!$this->form_validation->run()) {
                ...
                if ($success) {
                    $this->response(['message' => _l('invoice_payment_recorded')], RestController::HTTP_OK);
                } else {
                    $this->response(['message' => _l('invoice_payment_record_failed')], RestController::HTTP_NOT_FOUND);
                }
            }
