...
class MediaModal {
	...
	showUploadLoading() {
		document.querySelector(
			"#MediaModal .upload-collapse .status"
		).innerHTML = `
	...
	save() {
		let file =
			document.querySelector("#MediaModal .files input:checked").value ??
			false;
		let src = file;
		...
		this.fileList.addEventListener("click", function (e) {
			...
			if (el) {
				...
				if (_this.filemanager.classList.contains("searching")) {
					...
					_this.filemanager.classList.remove("searching");
					let search =
						_this.filemanager.querySelector("input[type=search]");
					search.val("");
					search.style.display = "none";
					_this.filemanager
	...
	onUpload(event) {
		...
		function imageIsLoaded(e) {
			...
			document.dispatchEvent(
				new CustomEvent("before-fetch", {detail: formData})
			);
			fetch(Vvveb.mediaUploadUrl, {method: "POST", body: formData})
				.then((response) => {
	...
	deleteFile(el) {
		...
		if (confirm(`Are you sure you want to delete "${file}"template?`)) {
			fetch(deleteUrl, {method: "POST", body: formData})
				...
				.then((data) => {
					displayToast("bg-success", "Success", data);
	...
	renameFile(el) {
		...
		if (newfile) {
			...
			fetch(renameUrl + "/media", {method: "POST", body: formData})
				.then((response) => {
					console.log(response);
					if (!response.ok) {
						throw new Error(response);
				...
				.then((data) => {
					displayToast("bg-success", "Success", data);
					parent.querySelector(".name").textContent = newfile
						.split("/")
						.reverse()[0];
