class OpenVerse {
	constructor() {
		...
		(this.filters = {
			...
			(this.baseUrl =
				"https://api.openverse.engineering/v1/images?format=json&filter_dead=true&");
		...
		fetch(url, {
			...
			.catch((error) => {
				...
				displayToast(
					"bg-danger",
					"Error",
					"Openverse authentication failed!"
				);
			});
	}
...
class OpenVerseDisplay extends OpenVerse {
	...
	setFilters() {
		this.filtersParameters = document
			.querySelector("#openverse-form")
			.serialize();
	...
	displayResults(data) {
		...
		document.getElementById("openverse-results").innerHTML =
			generateElements("<div>" + pagination + "</div>")[0];
	}

	page(pageNo) {
		this.pageNo = pageNo;
		this.filtersParameters =
			new FormData("openverse-form").toString() + "&page=" + pageNo;
		this.showLoading();
	...
	displayPanel() {
		return generateElements(
			`<ul class="data" id="openverse-results"></ul>`
		)[0];
	}
	...
	init() {
		...
		document
			.querySelector("#MediaModal .top-panel")
			.append(self.topPanel());
		document
			.querySelector("#MediaModal .display-panel")
			.append(self.displayPanel());
		document
			.querySelector("#MediaModal .top-right .align-right")
			.append(self.toggleBtn());
		document
			.querySelector("#MediaModal .modal-footer .align-left")
			.append(self.paginationContainer());
		document
			.querySelector("#openverse-filters")
			.prepend(self.getFiltersHtml());
		document.querySelector("#openverse-search-btn").click(function (e) {
			self.search();
			e.preventDefault();
		});
		...
		document
			...
			.on("show.bs.collapse", function (e) {
				if (e.target.id == "openverse-form") {
					document
						.querySelector("#MediaModal #openverse-results")
						.show();
					document
						.querySelector("#MediaModal #openverse-pagination")
						.show();
