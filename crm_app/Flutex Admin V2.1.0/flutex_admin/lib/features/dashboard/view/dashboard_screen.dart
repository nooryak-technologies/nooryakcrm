                    ...
                    child: Column(
                      children: [
                        ...
                        Row(
                          children: [
                            DashboardCard(
                              currentValue: '${controller.homeModel.overview?.currencySymbol ?? ''}${controller.homeModel.overview?.advancePaidTotal ?? '0'}',
                              totalValue: controller.homeModel.overview?.advancePaidCount ?? '0',
                              percent: (((double.tryParse(controller.homeModel.overview?.advancePaidCount ?? '0') ?? 0) / (double.tryParse(controller.homeModel.overview?.totalInvoices ?? '1') ?? 1)) * 100).toStringAsFixed(2),
                              icon: Icons.payments_outlined,
                              title: LocalStrings.advancePaid.tr,
                              onTap: () => Get.toNamed(RouteHelper.invoiceScreen),
                            ),
                            DashboardCard(
                              currentValue: '${controller.homeModel.overview?.currencySymbol ?? ''}${controller.homeModel.overview?.pendingPaymentTotal ?? '0'}',
                              totalValue: controller.homeModel.overview?.pendingPaymentCount ?? '0',
                              percent: (((double.tryParse(controller.homeModel.overview?.pendingPaymentCount ?? '0') ?? 0) / (double.tryParse(controller.homeModel.overview?.totalInvoices ?? '1') ?? 1)) * 100).toStringAsFixed(2),
                              icon: Icons.pending_actions_outlined,
                              title: LocalStrings.pendingPayment.tr,
                              onTap: () => Get.toNamed(RouteHelper.invoiceScreen),
                            ),
                          ],
                        ),
                        ...
                        if ((controller.homeModel.menuItems?.projects ?? false) &&
                            controller.homeModel.data?.projects != null &&
                            controller.homeModel.data!.projects!.isNotEmpty)
                          Column(
                            children: [
                              Padding(
