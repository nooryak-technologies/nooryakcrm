...
class Invoices_model extends App_Model
{
    ...
    public function get_invoices_total($data)
    {
        ...
        $result['advance_paid_count'] = 0;
        $result['pending_payment_count'] = 0;
        ...
        $noPermissionsQuery                 = get_invoices_where_sql_for_staff(get_staff_user_id());
            ...
            if ($dateRange === 'today') {
                ...
                $fromDate = to_sql_date($dateFrom);
                $toDate    = to_sql_date($dateTo);
        ...
        for ($i = 1; $i <= 5; $i++) {
            ...
            if ($i == 1) {
                ...
                $select = 'id,total,COALESCE(advance, 0) as advance';
            }
            ...
            if ($i == 3) {
                ...
                $this->db->where('advance >', 0);
            } elseif ($i == 5) {
                $this->db->where('total > COALESCE(advance, 0)');
            } elseif ($i == 1) {
                $this->db->where('status !=', self::STATUS_PAID);
            }

            if ($dateRange === '' && isset($data['years']) && count($data['years']) > 0) {
            ...
            foreach ($invoices as $invoice) {
                if ($i == 1) {
                    ...
                    $result['advance_paid'][] = $invoice['advance'];
                } elseif ($i == 5) {
                    $result['pending_payment'][] = (float) $invoice['total'] - (float) $invoice['advance'];
                }
            }

            if ($i == 4) {
                $result['advance_paid_count'] = count($invoices);
            } elseif ($i == 5) {
                $result['pending_payment_count'] = count($invoices);
            }
        }
        ...
        $result['advance_paid'] = array_sum($result['advance_paid']);
        $result['pending_payment'] = array_sum($result['pending_payment']);
    ...
    public function add($data, $expense = false)
    {
        ...
        // Remove duration field if present (belongs to items, not invoice table)
    ...
    public function copy($id)
    {
        ...
        $new_invoice_data['advance']          = $_invoice->advance;
