...
trait PDF_Signature
{
    ...
    public function process_signature()
    {
        ...
        if ($companySignature) {
            // Render only the signature line or image without the "Authorized Signature" label
            $companySignatureHtml = '<div nobr="true">' . $companySignature . '</div>';
    ...
    public function getCompanySignature($lineBreaksBefore = 1)
    {
        ...
        || ($this->type() == 'credit_note') && get_option('show_pdf_signature_credit_note') == 1) {
            ...
            // Default to empty so no blank signature line is rendered when no signature image is provided.
            $blankSignatureLine = hooks()->apply_filters('blank_signature_line', '');
