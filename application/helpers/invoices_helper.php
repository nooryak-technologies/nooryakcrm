...
function get_invoice_total_left_to_pay($id, $invoice_total = null)
{
    ...
    if ($invoice_total === null) {
        $CI->db->select('total, COALESCE(advance, 0) as advance')
            ->where('id', $id);
        $invoice_data = $CI->db->get(db_prefix() . 'invoices')->row();
        $invoice_total = $invoice_data->total;
        $invoice_advance = (float) $invoice_data->advance;
    } else {
        $CI->db->select('COALESCE(advance, 0) as advance')
            ->where('id', $id);
        $invoice_data = $CI->db->get(db_prefix() . 'invoices')->row();
        if ($invoice_data) {
            $invoice_advance = (float) $invoice_data->advance;
    ...
    $baseTotal = (float) $invoice_total - $invoice_advance;
    ...
    if (function_exists('bcsub')) {
        return bcsub((string) $baseTotal, (string) $totalPayments, get_decimal_places());
    }

    return number_format($baseTotal - $totalPayments, get_decimal_places(), '.', '');
}
...
function get_invoices_where_sql_for_staff($staff_id)
{
    ...
    if ($has_permission_view_own) {
        ...
        // Add filter to show invoices from leads assigned to this staff member
        $whereUser .= ' OR ' . db_prefix() . 'invoices.clientid IN (SELECT ' . db_prefix() . 'clients.userid FROM ' . db_prefix() . 'clients WHERE ' . db_prefix() . 'clients.leadid IN (SELECT id FROM ' . db_prefix() . 'leads WHERE addedfrom=' . $CI->db->escape_str($staff_id) . ' OR assigned=' . $CI->db->escape_str($staff_id) . '))';
        $whereUser .= ')';
    } else {
...
function user_can_view_invoice($id, $staff_id = false)
{
    ...
    $CI->db->select('id, addedfrom, sale_agent, clientid');
    ...
    // Check if invoice is from a lead assigned/added by this staff member
    $CI->db->select('COUNT(*) as count');
    $CI->db->from(db_prefix() . 'clients');
    $CI->db->join(db_prefix() . 'leads', db_prefix() . 'leads.id = ' . db_prefix() . 'clients.leadid', 'inner');
    $CI->db->where(db_prefix() . 'clients.userid', $invoice->clientid);
    $CI->db->where('(' . db_prefix() . 'leads.addedfrom=' . $CI->db->escape_str($staff_id) . ' OR ' . db_prefix() . 'leads.assigned=' . $CI->db->escape_str($staff_id) . ')');
