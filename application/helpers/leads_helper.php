...
/**
 * Render lead tags dropdown with predefined options
 * @param  string  $selected        selected tag
 * @param  string  $lang_key        the label of the select
 * @param  string  $name            the name of the select
 * @param  array   $select_attrs    additional select attributes
 * @return string
 */
function render_lead_tags_select($selected = '', $lang_key = 'tags', $name = 'tags', $select_attrs = [])
{
    $tag_options = [
        [
            'id'   => 'Cold',
    ...
    // Allow blank selection (no tag)
    return render_select($name, $tag_options, ['id', 'name'], $lang_key, $selected, $select_attrs);
}
