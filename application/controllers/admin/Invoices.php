...
class Invoices extends AdminController
{
    ...
    public function invoice($id = '')
    {
        if ($this->input->post()) {
            ...
            // If invoice is created for a lead, convert or reuse client first
            if (isset($invoice_data['leadid']) && $invoice_data['leadid'] != '') {
                $leadid = $invoice_data['leadid'];
                // Check if a client already exists for this lead
                $existing_client = $this->db->select('userid')->from(db_prefix() . 'clients')->where('leadid', $leadid)->get()->row();
                if ($existing_client && isset($existing_client->userid) && $existing_client->userid != '') {
                    $invoice_data['clientid'] = $existing_client->userid;
                } else {
                    // Create a minimal client from lead data
                    ...
                    if ($lead) {
                        ...
                        if ($new_client_id) {
                            $invoice_data['clientid'] = $new_client_id;
                        }
                    }
                }

                // Remove leadid from invoice data so invoices_model->add doesn't choke
                unset($invoice_data['leadid']);
            }
            if ($id == '') {
                if (staff_cant('create', 'invoices')) {
                    access_denied('invoices');
        ...
        // If creating invoice for leads only mode, provide leads list to the view
        if ($this->input->get('lead_mode') || $this->input->get('lead_id')) {
            $this->load->model('leads_model');
            $data['lead_mode'] = true;
            $data['leads']     = $this->leads_model->get();
            $data['selected_lead'] = $this->input->get('lead_id') ? $this->input->get('lead_id') : '';
        }
