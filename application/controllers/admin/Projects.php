...
class Projects extends AdminController
{
    ...
    public function project($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            $data                = $this->normalize_project_customer_selection($data);
            if ($id == '') {
        ...
        $data['project_customer_options'] = $this->get_project_customer_options();
        $data['project_customer_selected'] = isset($data['project']) ? 'customer_' . $data['project']->clientid : ($this->input->get('customer_id') ? 'customer_' . $this->input->get('customer_id') : '');
    ...
    private function normalize_project_customer_selection(array $data)
    {
        if (! isset($data['clientid']) || $data['clientid'] === '') {
            return $data;
        }

        $selected = (string) $data['clientid'];

        if (strpos($selected, 'customer_') === 0) {
            $data['clientid'] = (int) substr($selected, 9);

            return $data;
        }

        if (strpos($selected, 'lead_') === 0) {
            $lead_id = (int) substr($selected, 5);

            if ($lead_id > 0) {
                $existing_client = $this->db->select('userid')->from(db_prefix() . 'clients')->where('leadid', $lead_id)->get()->row();
                if ($existing_client && isset($existing_client->userid) && $existing_client->userid != '') {
                    $data['clientid'] = (int) $existing_client->userid;
                ...
                if ($lead) {
                    ...
                    $new_client_id = $this->clients_model->add($clientData, true);
                    if ($new_client_id) {
    ...
    private function get_project_customer_options()
    {
        $options = [];
        ...
        foreach ($customers as $customer) {
            $options[] = [
                'id'      => 'customer_' . $customer['userid'],
                'name'    => $customer['company'],
                'subtext' => 'Customer',
            ];
        }

        $leads = $this->leads_model->get('', ['junk' => 0]);
        foreach ($leads as $lead) {
            $options[] = [
                'id'      => 'lead_' . $lead['id'],
                'name'    => $lead['name'] . (! empty($lead['email']) ? ' - ' . $lead['email'] : ''),
                'subtext' => 'Lead',
            ];
        }
