...
<div class="lead-wrapper<?= $openEdit == true ? ' open-edit' : ''; ?>"
    ...
    <div class="tw-flex tw-items-center tw-justify-end tw-space-x-1.5">
        ...
        <a data-toggle="tooltip" class="btn btn-success lead-top-btn lead-view" data-placement="top"
            title="<?= _l('create_new_invoice'); ?>"
            href="<?= $client ? admin_url('invoices/invoice?customer_id=' . get_client_id_by_lead_id($lead->id)) : admin_url('invoices/invoice?lead_mode=1&lead_id=' . $lead->id); ?>">
...
// If staff member is adding a new lead, show assigned staff name as text instead of dropdown
if (!isset($lead) && !is_admin()) {
    ...
    <div class="form-group">
        ...
        <input type="text" class="form-control" value="<?= e($current_staff_name); ?>" disabled>
        <input type="hidden" name="assigned" value="<?= e($current_staff_id); ?>">
    </div>
    <?php
} else {
...
?>
            ...
            <div class="col-md-4">
                <?php
                    // Get the first tag if exists
                    $selected_tag = '';
                    if (isset($lead)) {
                        $tags = get_tags_in($lead->id, 'lead');
                        if (!empty($tags) && is_array($tags) && count($tags) > 0) {
                            $selected_tag = $tags[0]; // Get first tag
                        }
                    }
                    echo render_lead_tags_select($selected_tag, 'tags');
                ?>
            </div>
            <div class="clearfix"></div>
            ...
            <div class="col-md-6">
                ...
                <?php if ((isset($lead) && empty($lead->website)) || ! isset($lead)) {
                    ...
                    echo render_input('website', 'Interested Service', $value);
                } else { ?>
                <div class="form-group">
                    <label
                        for="website">Interested Service</label>
