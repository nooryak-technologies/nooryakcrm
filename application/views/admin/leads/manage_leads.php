        ...
        <div class="row">
            <div class="col-md-12">
                <div
                    ...
                    <style>
                        .leads-overview button, 
                        .leads-overview .btn,
                        .leads-overview .bootstrap-select button {
                        ...
                        .btn-active-filter .caret {
                            color: #fff !important;
                        }
                        .btn-active-filter .filter-option {
                            color: #fff !important;
                        }
                    </style>
                    <div class="tw-grid tw-gap-2 sm:tw-grid-flow-col sm:tw-auto-cols-max tw-overflow-x-auto">
                        ...
                        <button type="button"
                            @click="extra.leadsStatus = '<?= isset($status['junk']) ? 'junk' : 'lost'; ?>'; extra.leadsRules = <?= app\services\utilities\Js::from($table->findRule(isset($status['junk']) ? 'junk' : 'lost')->setValue(true)); ?>"
                            class="tw-bg-transparent tw-border tw-border-solid tw-border-danger tw-shadow-sm tw-py-1 tw-px-2 tw-rounded-lg tw-text-sm hover:tw-bg-danger/10 tw-text-danger text-left"
                            :class="extra.leadsStatus === '<?= isset($status['junk']) ? 'junk' : 'lost'; ?>' ? '!tw-bg-black !tw-border-black !tw-text-white' : ''">
                        ...
                        <?php } else { ?>
                        <button type="button"
                            @click="extra.leadsStatus = <?= (int) $status['id']; ?>; extra.leadsRules = <?= app\services\utilities\Js::from($table->findRule('status')->setValue([$status['id']])); ?>"
                            ...
                            :class="extra.leadsStatus === <?= (int) $status['id']; ?> ? '!tw-bg-black !tw-border-black !tw-text-white' : ''">
                            <span class="tw-font-semibold tw-mr-1 rtl:tw-ml-1">
                                <?= e($status['total']); ?>
                            </span>
                            <span
                                :class="extra.leadsStatus === <?= (int) $status['id']; ?> ? '!tw-text-white' : ''"
                ...
                <div class="_buttons tw-mb-2">
                    <div class="tw-flex tw-items-center tw-justify-between tw-space-x-2 rtl:tw-space-x-reverse">
                        <div class="tw-flex tw-items-center tw-space-x-1 rtl:tw-space-x-reverse">
                            ...
                            <button type="button"
                                class="hidden-xs btn btn-default"
                                @click="extra.leadsStatus = null; extra.leadsRules = [];">
                        ...
                        <div class="tw-flex tw-items-center tw-space-x-2">
                            ...
                            <div class="tw-inline-block">
                                <select name="custom_view" id="custom_view" class="selectpicker" data-width="150px" data-none-selected-text="Filter Date" onchange="if(this.value=='custom'){$('#custom_date_range').removeClass('hide');}else{$('#custom_date_range').addClass('hide');} $('.table-leads').DataTable().ajax.reload();">
                            ...
                            <div id="custom_date_range" class="hide tw-inline-flex tw-items-center tw-space-x-1">
                                <input type="date" name="custom_view_from" id="custom_view_from" class="form-control tw-w-36" onchange="$('.table-leads').DataTable().ajax.reload();" placeholder="From">
                                <input type="date" name="custom_view_to" id="custom_view_to" class="form-control tw-w-36" onchange="$('.table-leads').DataTable().ajax.reload();" placeholder="To">
                            </div>
                        ...
                        if ($isKanBan) { ?>
                            ...
                            <div class="row" id="leads-table">
                              ...
                              $table_data    = [];
                                ...
                                $_table_data[] = [
                                    'name'     => _l('total_advance'),
                                    'th_attrs' => ['class' => 'toggleable', 'id' => 'th-total-advance'],
                                ];
                                $_table_data[] = [
                                    'name'     => _l('pending_payment'),
                                    'th_attrs' => ['class' => 'toggleable', 'id' => 'th-pending-payment'],
                                ];
                                $_table_data[] = [
                                ...
                                $_table_data[] = [
                                    'name'     => '<div class="audio-update-th-container"><i class="fa-solid fa-microphone text-primary"></i><span>Audio Update</span></div>',
                                    'th_attrs' => ['class' => 'toggleable text-center', 'id' => 'th-chat'],
                                ];
                                $_table_data[] = [
...
<?php init_tail(); ?>
<?php $this->load->view('admin/leads/leads_chat_modal'); ?>
<script>
    var openLeadID = '<?= e($leadid); ?>';
    $(function() {
        if (typeof extra !== 'undefined') {
            extra.leadsStatus = <?= $this->input->get('status') ? (int) $this->input->get('status') : 'null'; ?>;
        }
        leads_kanban();
        ...
        $(document).on('preXhr.dt', function (e, settings, data) {
            if (settings.nTable.id == 'leads') {
                data.custom_view_from = $('#custom_view_from').val();
                data.custom_view_to = $('#custom_view_to').val();
            }
        });

        // Handle active state for custom_view dropdown
        $('#custom_view').on('change', function() {
            var $parent = $(this).closest('.bootstrap-select');
            if ($(this).val() !== '') {
                $parent.find('button').addClass('btn-active-filter');
            } else {
                $parent.find('button').removeClass('btn-active-filter');
            }
        });
        
        // Initial check for custom_view
        if ($('#custom_view').val() !== '') {
            $('#custom_view').closest('.bootstrap-select').find('button').addClass('btn-active-filter');
        }

        // Color picker live custom updates for Kanban status pills
        $(document).on('click', '.kanban-cpicker', function() {
            ...
            if ($popover.length) {
                ...
                if ($trigger.length) {
                    $heading = $trigger.closest('.panel-heading');
                }
            }
            if (!$heading || !$heading.length) {
                $heading = $(this).parents('.panel-heading');
            }
            
            if ($heading && $heading.length) {
                var $pill = $heading.find('.kanban-status-pill');
                var $val = $heading.find('.kanban-status-value');
                if ($pill.length) {
                    $pill.css({
                        'color': '#ffffff',
                    ...
                    $pill.find('i, .status-name, .status-count').css('color', '#ffffff');
                }
                if ($val.length) {
                    $val.css('color', color);
                }
                // Override default panel heading color application applied by core JS
                setTimeout(function() {
                    $heading.css({
        ...
        // Drag-and-drop dynamic target column dashed border highlights (sortstart/sortover/sortout/sortstop)
        $(document).on('sortstart', function(event, ui) {
            $('.leads-status').removeClass('ui-sortable-hover');
        });

        $(document).on('sortover', '.leads-status', function(event, ui) {
            $('.leads-status').removeClass('ui-sortable-hover');
            $(this).addClass('ui-sortable-hover');
        });
        
        $(document).on('sortout', '.leads-status', function(event, ui) {
    ...
    // Global Load More local pagination logic
    window.load_more_local_leads = function(statusId, button) {
        var $column = $('[data-lead-status-id="' + statusId + '"]');
        var visibleCount = parseInt($column.attr('data-visible-count') || '4');
        var newVisibleCount = visibleCount + 2;
        $column.attr('data-visible-count', newVisibleCount);
        
        var $allCards = $column.find('> li.lead-kan-ban');
        
        // Load and show the next 2 hidden cards
        $allCards.slice(0, newVisibleCount).slideDown(200).removeClass('hidden-lead-card');
        
        // Hide Load More button if no more cards are left to show
        if ($allCards.length <= newVisibleCount) {
            $(button).closest('.custom-kanban-load-more').addClass('hide');
        }
    };
</script>
</body>
