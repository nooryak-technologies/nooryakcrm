      ...
      foreach ($custom_fields as $field) {
          ...
          if ($value == '') {
              ...
              if (!in_array($c_field, $not_mergable_contact_fields)) {
                ...
                <?php echo form_hidden('redirect_to_invoice', ''); ?>
            ...
            <div class="modal-footer">
                ...
                <button type="button" id="convert-and-invoice" autocomplete="off"
                    data-loading-text="<?php echo _l('wait_text'); ?>"
                    class="btn btn-primary"><?php echo _l('convert_to_client') . ' & ' . _l('create_new_invoice'); ?></button>
...
init_selectpicker();
$('#convert-and-invoice').on('click', function() {
    $('input[name="redirect_to_invoice"]').val('1');
    $('#lead_to_client_form').submit();
});
// Ensure the hidden field is cleared on manual submit/back
$('#lead_to_client_form').on('submit', function() {
    // keep value if set by convert-and-invoice, otherwise ensure empty
    if ($('input[name="redirect_to_invoice"]').val() != '1') {
        $('input[name="redirect_to_invoice"]').val('');
    }
});
</script>