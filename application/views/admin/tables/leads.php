...
return App_table::find('leads')
    ->outputUsing(function ($params) use ($statuses) {
        ...
        $aColumns = array_merge($aColumns, [
            ...
            '(SELECT COALESCE(SUM(advance), 0) FROM ' . db_prefix() . 'invoices WHERE clientid IN (SELECT userid FROM ' . db_prefix() . 'clients WHERE leadid = ' . db_prefix() . 'leads.id) AND status != 5) as total_advance',
            '(SELECT COALESCE(SUM(total) - COALESCE(SUM(advance), 0), 0) FROM ' . db_prefix() . 'invoices WHERE clientid IN (SELECT userid FROM ' . db_prefix() . 'clients WHERE leadid = ' . db_prefix() . 'leads.id) AND status != 5) as pending_payment',
        ...
        if ($this->ci->input->post('custom_view')) {
            ...
            if ($filter == 'today') {
                $where[] = 'AND DATE(' . db_prefix() . 'leads.dateadded) = "' . date('Y-m-d') . '"';
            } elseif ($filter == 'this_week') {
                $where[] = 'AND DATE(' . db_prefix() . 'leads.dateadded) >= "' . date('Y-m-d', strtotime('monday this week')) . '" AND DATE(' . db_prefix() . 'leads.dateadded) <= "' . date('Y-m-d', strtotime('sunday this week')) . '"';
            } elseif ($filter == 'this_month') {
                $where[] = 'AND DATE(' . db_prefix() . 'leads.dateadded) >= "' . date('Y-m-01') . '" AND DATE(' . db_prefix() . 'leads.dateadded) <= "' . date('Y-m-t') . '"';
            } elseif ($filter == 'custom') {
                $from = $this->ci->input->post('custom_view_from');
                $to = $this->ci->input->post('custom_view_to');
                if ($from && $to) {
                    $where[] = 'AND DATE(' . db_prefix() . 'leads.dateadded) >= "' . $from . '" AND DATE(' . db_prefix() . 'leads.dateadded) <= "' . $to . '"';
                }
            }
        }
        ...
        foreach ($rResult as $aRow) {
            ...
            // Add Advance column
            $row[] = '<span class="tw-font-medium">' . e(($aRow['total_advance'] != 0 ? app_format_money($aRow['total_advance'], $base_currency->id) : '')) . '</span>';

            // Add Pending Payment column
            $row[] = '<span class="tw-font-medium">' . e(($aRow['pending_payment'] != 0 ? app_format_money($aRow['pending_payment'], $base_currency->id) : '')) . '</span>';
            ...
            // Insert "Audio Update" button column cell directly after "Source"
            $row[] = '<div style="display: flex; align-items: center; justify-content: center; width: 100%;"><button type="button" class="btn btn-default btn-icon chat-lead-btn" data-lead-id="' . $aRow['id'] . '" data-lead-name="' . e($aRow['name']) . '" title="Audio Update" style="display: inline-flex; align-items: center; justify-content: center; padding: 4px 8px; font-size: 14px; margin: 0 auto;"><i class="fa-solid fa-microphone text-primary"></i></button></div>';
