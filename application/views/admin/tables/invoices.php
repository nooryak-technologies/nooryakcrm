...
return App_table::find('invoices')
    ->outputUsing(function ($params) {
        ...
        foreach ($rResult as $aRow) {
            ...
            $row[] = '<span class="tw-font-medium">' . e(app_format_money($aRow['advance'], $aRow['currency_name'])) . '</span>';

            $pendingPayment = max(0, (float) $aRow['total'] - (float) $aRow['advance']);
            $row[] = '<span class="tw-font-medium">' . e(app_format_money($pendingPayment, $aRow['currency_name'])) . '</span>';
        ...
        App_table_filter::new('pending_payment', 'BooleanRule')->label(_l('pending_payment'))->raw(function ($value) {
            return $value == '1'
                ? '(total > COALESCE(advance, 0))'
                : '(total <= COALESCE(advance, 0))';
        }),
        App_table_filter::new('advance_paid', 'BooleanRule')->label(_l('advance_paid'))->raw(function ($value) {
            return $value == '1'
                ? 'COALESCE(advance, 0) > 0'
                : 'COALESCE(advance, 0) = 0';
        }),
