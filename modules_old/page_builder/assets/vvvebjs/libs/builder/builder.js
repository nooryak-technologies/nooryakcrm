	...
	render: function (type, panel = false) {
		...
		for (let i in component.properties) {
			...
			if (property.inputtype == SectionInput) {
				section = componentsPanelSections[
					propertySection
				].querySelector(
...
Vvveb.WysiwygEditor = {
	...
	edit: function (element) {
		...
		document.getElementById("fore-color").value =
			Vvveb.StyleManager.getStyle(element, "color");
		document.getElementById("back-color").value =
			Vvveb.StyleManager.getStyle(element, "background-color");
		element.focus();
	},
...
Vvveb.Builder = {
		...
		let highlightMove = function (event) {
			if (
				...
				if (self.isResize) {
					...
					try {
						if (
							pos.top < y - halfHeight ||
							pos.left < x - halfWidth
						) {
						...
						if (self.designerMode) {
							let parentOffset = offset(
								self.dragElement.offsetParent
							);
							self.dragElement.style.position = "absolute";
							self.dragElement.style.x =
								x -
								(parentOffset.left - self.frameDoc.scrollLeft);
							self.dragElement.style.y =
								y -
								(parentOffset.top - self.frameDoc.scrollTop);
						}
					} catch (err) {
						console.log(err);
						return false;
					...
					if (!self.designerMode && self.iconDrag) {
						...
						self.iconDrag.style.left =
							x + self.leftPanelWidth + 10 + "px";
					}
				} // else //uncomment else to disable parent highlighting when dragging
				{
					...
					document.getElementById("highlight-box").setAttribute(
						...
						`top:${pos.top - (self.frameDoc.scrollTop ?? 0)}px; 
                           ...
                           border:${
								self.isDragging ? "1px dashed #0d6efd" : ""
							};
                      `
					);
		...
		let highlightUp = function (event) {
			...
			if (self.isDragging) {
				...
				if (propertiesTab.offsetParent) {
					...
					const bsTab =
						bootstrap.Tab.getOrCreateInstance(propertiesTab);
					bsTab.show();
				}
				...
				if (!Vvveb.WysiwygEditor.isActive) {
					...
					_updateSelectBox = function (event) {
						...
						SelectBox.style.left =
							...
							"px";
						SelectBox.style.width =
							self.texteditEl.offsetWidth +
							self.selectPadding * 2 +
							"px";
						SelectBox.style.height =
							self.texteditEl.offsetHeight +
							self.selectPadding * 2 +
							"px";
						SelectBox.style.display = "block";
					};
			...
			if (Vvveb.Builder.isPreview == false) {
				if (event.target) {
					...
					if (componentTab.offsetParent) {
						...
						const bsTab =
							bootstrap.Tab.getOrCreateInstance(componentTab);
						bsTab.show();
					}
	...
	_initBox: function () {
		...
		document
			...
			.addEventListener("click", function (event) {
				...
				if (top < 0) top = 0;
				if (
					left + addSectionBox.offsetWidth >
					self.frameHtml.offsetWidth
				)
					left =
						self.frameHtml.offsetWidth - addSectionBox.offsetWidth;
				if (
					top +
						addSectionBox.offsetHeight +
						self.frameHtml.scrollTop >
					outerHeight
				)
	...
	getHtml: function (keepHelperAttributes = true) {
		...
		doc.querySelectorAll("[spellcheckker]").forEach((e) =>
			e.removeAttribute("spellcheckker")
		);
...
Vvveb.Gui = {
	init: function () {
		document
			.querySelectorAll("[data-vvveb-action]")
	...
	saveAjax: function () {
		...
		if (btn.classList.contains("save-offcanvas")) {
			if (
				document.querySelector(
					"#save-offcanvas [name=template]:checked"
				).value == "new"
			) {
				file =
					document.querySelector("#save-offcanvas [name=folder]")
						.value +
					"/" +
		...
		return Vvveb.Builder.saveAjax(
			...
			(error) => {
				...
				let message =
					error?.responseText ?? error?.statusText ?? "Error saving!";
				displayToast("bg-danger", "Error", message);
			}
		);
	},
	...
	viewport: function () {
		document
			...
			.setAttribute("class", this.dataset.view);
	...
	toggleEditor: function () {
		...
		document
			.getElementById("toggleEditorJsExecute")
	...
	search: function () {
		...
		panel.querySelectorAll("li ol li").forEach(function (el, i) {
			...
			if (el.dataset.search.indexOf(searchText) > -1)
				el.style.display = "";
		});
	},
	...
	toggleTreeList: function () {
		...
		if (!treeList.offsetParent) {
			document
				.getElementById("toggle-tree-list")
			...
			.addEventListener("click", function (e) {
				...
				if (element) {
					...
					if (afterSection) {
						...
						if (sectionType == "nav") {
							afterSection =
								Vvveb.Builder.frameBody.querySelector(
				...
				fetch(renameUrl, {
					...
					.then((data) => {
						...
						if (duplicate) {
							...
							page.url =
								data.url ??
								page.url.replace(page.file, newfile);
							page.file = newfile;
					...
					.catch((error) => {
						console.log(error);
						let message =
							error?.statusText ?? "Error renaming page!";
						displayToast("bg-danger", "Error", message);
	...
	addPage: function (name, data, afterPage = false) {
		...
		if (data.folder) {
			if (
				...
				folder = generateElements(
					tmpl("vvveb-filemanager-folder", data)
				)[0];
		...
		if (
			afterPage &&
			(afterPage = folder.querySelector(
				'[data-page="' + afterPage + '"]'
			))
		) {
...
Vvveb.Breadcrumb = {
	...
	addElement: function (data, element) {
		let li = generateElements(
			tmpl("vvveb-breadcrumb-navigaton-item", data)
		)[0];
		li._node = element;
