...
Vvveb.Components.add("elements/tabs", {
	...
	afterDrop: function (node) {
		...
		node.outerHTML = node.outerHTML.replaceAll(
			"parentId",
			...
			onChange: function (node, value, input, component, event) {
				...
				if (event.action) {
					if (event.action == "add") {
						...
						content.append(
							generateElements(
								`<div class="tab-pane p-4" id="tab-${tabsId}-${index}-${random}" role="tabpanel" aria-labelledby="tab-${tabsId}-${index}-${random}" tabindex="0"><p>Quisque sagittis non ex eget vestibulum</p></div>`
							)[0]
						);
					...
					if (event.action == "remove") {
						nav.querySelector(
							"> button:nth-child(" + event.index + ")"
						).remove();
						content
							.querySelector(
								"> .tab-pane:nth-child(" + event.index + ")"
							)
							.remove();
						...
						Vvveb.Builder.iframe.contentWindow.bootstrap.Tab.getOrCreateInstance(
							tab
						).show();
					}
				}
			...
			onChange: function (node, value, input, component, event) {
				...
				if (event.action) {
					if (event.action == "add") {
						...
						node.append(
							generateElements(`<div class="accordion-item">
					...
					if (event.action == "remove") {
						node.querySelector(
							":scope > .accordion-item:nth-child(" +
								event.index +
								")"
						).remove();
					} else if (event.action == "select") {
