...
Vvveb.Components.extend("_base", "html/video", {
	...
	name: "Video",
	image: "icons/video.svg",
	html: `<video class="full" controls playsinline src="${Vvveb.assetBaseUrl}/media/5.jpg" poster="${Vvveb.assetBaseUrl}/media/5.jpg"></video>`,
	properties: [
		{
			name: "Poster",
			key: "poster",
...
Vvveb.Components.extend("_base", "html/html", {
	...
	onChange: (node, property, value) => {
		let cleanedValue = DOMPurify.sanitize(value);
		for (let i = 0; i < node.length; i++) {
			const n = node[i];
			property.htmlAttr === "innerText"
				? (n.textContent = cleanedValue)
				: n.setAttribute(property.htmlAttr, cleanedValue);
		}
	},
	properties: [
		{
		...
		},
		{
			...
			htmlAttr: "innerText",
			inputtype: TextareaInput,
			child: "style[id='meta-custom-styles']",
		},
	],
});
