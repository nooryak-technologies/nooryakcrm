...
function fix_kanban_height(col_px, container_px) {
  ...
  $("#kan-ban").closest(".container-fluid").css("min-width", kanbanColCount * container_px + "px");
}
...
function parse_money_value(str) {
...
function format_money_value(val, symbol) {
  if (val === 0) return symbol + "0.00";
  var formatted = val.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
  return symbol + formatted;
}

function update_kan_ban_ui_instantly(ui, newStatus) {
  if (ui.sender) {
    var $oldCol = $(ui.sender).closest(".kan-ban-col");
    var $newCol = $(ui.item).closest(".kan-ban-col");
    
    if ($oldCol.attr("data-col-status-id") != newStatus) {
      // 1. Update data-total attributes
      var oldTotal = parseInt($oldCol.attr("data-total")) || 0;
      var newTotal = parseInt($newCol.attr("data-total")) || 0;
      $oldCol.attr("data-total", Math.max(0, oldTotal - 1));
      $newCol.attr("data-total", newTotal + 1);

      // 2. Update status counts in the header UI
      $oldCol.find(".status-count").text(Math.max(0, oldTotal - 1));
      $newCol.find(".status-count").text(newTotal + 1);

      // 3. Update kanban-empty visibility
      if (Math.max(0, oldTotal - 1) === 0) {
        $oldCol.find(".kanban-empty").removeClass("hide");
      } else {
        $oldCol.find(".kanban-empty").addClass("hide");
      }
      $newCol.find(".kanban-empty").addClass("hide");

      // 4. Parse card money value
      var cardMoneyText = $(ui.item).find(".lead-value-badge").text();
      var cardMoneyVal = parse_money_value(cardMoneyText);

      if (cardMoneyVal > 0) {
        // 5. Update money values in the header UI
        var oldMoneyText = $oldCol.find(".kanban-status-value").text();
        var newMoneyText = $newCol.find(".kanban-status-value").text();
        
        var oldMoneyVal = parse_money_value(oldMoneyText);
        var newMoneyVal = parse_money_value(newMoneyText);

        var finalOldMoney = Math.max(0, oldMoneyVal - cardMoneyVal);
        var finalNewMoney = newMoneyVal + cardMoneyVal;

        // Detect symbol from text
        var symbol = oldMoneyText.replace(/[\d.,\s]/g, "") || "₹";

        $oldCol.find(".kanban-status-value").text(format_money_value(finalOldMoney, symbol));
        $newCol.find(".kanban-status-value").text(format_money_value(finalNewMoney, symbol));
      }
    }
  }
}
...
function leads_kanban_update(ui, object) {
  ...
  update_kan_ban_ui_instantly(ui, data.status);

  $.post(admin_url + "leads/update_lead_status", data).done(function (
    response
  ) {
    // Silent success - do not reload the board
  }).fail(function (err) {
    alert_float("danger", "Failed to update lead status");
    leads_kanban();
  });
}
...
function add_item_to_preview(id) {
  requestGetJSON("invoice_items/get_item_by_id/" + id).done(function (
    ...
    $('.main input[name="duration"]').val(response.duration);
...
function clear_item_preview_values(default_taxes) {
  ...
  previewArea.find('input[name="unit"]').val("");
  previewArea.find('input[name="duration"]').val("");
...
function add_item_to_table(data, itemid, merge_invoice, bill_expense) {
  ...
  get_taxes_dropdown_template(tax_name, data.taxname).done(function (
    ...
    table_row +=
      '<td><input type="text" placeholder="Duration" name="newitems[' +
      item_key +
      '][duration]" class="form-control" value="' +
      data.duration +
      '"></td>';
...
function get_item_preview_values() {
  ...
  response.duration = $('.main input[name="duration"]').val();
...
function init_invoices_total(manual) {
  ...
  var currency = _inv_total_wrapper.find('select[name="total_currency"]').val();
  var dateRange = _inv_total_wrapper.find('select[name="date_range"]').val();
  var dateFrom = _inv_total_wrapper.find('input[name="date_from"]').val();
  var dateTo = _inv_total_wrapper.find('input[name="date_to"]').val();
