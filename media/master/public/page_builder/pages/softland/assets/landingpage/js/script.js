/**
 * NOORYAK CRM – Landing Page Scripts v2
...
(function () {
    ...
    function handleScroll() {
        ...
        /* Keep sticky in sync so main.js + glass styles both apply */
...
/* ── Smart CRM Features – Tab Switcher + Panel Restructure ── */
(function () {
...
(function () {
    ...
    function buildCard(pkg, index, isPopular, interval) {
        ...
        var noteBlock = '';
        // Build user limit text for all plans
        var userLimit = pkg.user_limit;
        var userText = '';
        if (userLimit && userLimit > 0 && userLimit !== -1 && userLimit <= 999) {
            userText = userLimit + ' User' + (userLimit > 1 ? 's' : '');
        } else if (userLimit === -1) {
            userText = 'Unlimited Users';
        }

        if (isFreeTrial && pkg.trial_period > 0) {
            noteBlock = '<div class="plan-note">' + pkg.trial_period + '-Day Access'
                + (userText ? ' &bull; ' + userText : '') + '</div>';
        } else if (userText) {
            noteBlock = '<div class="plan-note">' + userText
                + (pkg.description ? ' &bull; ' + pkg.description : '') + '</div>';
        } else if (pkg.description) {
            noteBlock = '<div class="plan-note">' + pkg.description + '</div>';
        } else {
            noteBlock = '<div class="plan-note">&nbsp;</div>';
        }
        ...
        fetch(apiUrl)
            ...
            .then(function (data) {
                if (data && data.success && Array.isArray(data.packages)) {
                    ...
                    var input = document.getElementById('team-size-input');
                    if (input) {
                        input.addEventListener('input', syncTeamSizeHighlight);
                        syncTeamSizeHighlight();
                    }
                    initBillingToggle();
                }
...
/* ------------------------------------------
   NOORYAK BUSINESS IMPACT SECTIONS � JS
------------------------------------------ */
(function () {
    // Accordion toggle for Pipeline section
    function initNbiAccordion() {
        document.querySelectorAll('.nbi-acc-head').forEach(function (head) {
            head.addEventListener('click', function () {
                var item = head.closest('.nbi-acc-item');
                var body = item.querySelector('.nbi-acc-body');
                var arrow = head.querySelector('.nbi-acc-arrow');
                var isOpen = body.classList.contains('open');
                // close all
                document.querySelectorAll('.nbi-acc-body').forEach(function (b) { b.classList.remove('open'); });
                document.querySelectorAll('.nbi-acc-arrow').forEach(function (a) { a.classList.remove('open'); });
                if (!isOpen) {
    ...
    // Scroll-reveal fade-in for stat numbers (business section only)
    function initNbiCounterReveal() {
        var section = document.querySelector('.nbi-section-business');
        if (!section) return;
        var els = section.querySelectorAll('.nbi-stat-val, .nbi-engage-val, .nbi-growth-pct, .nbi-cust-val');
...
/* ═══════════════════════════════════════════════════════════
   TESTIMONIALS SLIDER – 3 on desktop, auto + manual scroll
═══════════════════════════════════════════════════════════ */
(function () {
    'use strict';
